...
function varianceHandler(sample, precise) {
  var out = null;
  var meaning = meanHandler(sample); 
  precise = (precise || false);

  function _mapHandler(item) {
    return Math.pow((item - meaning), 2); 
  }
  sample = sample.map(_mapHandler);

  function _reduceHandler(current, left) {
    return current + left;
  }
  out = sample.reduce(_reduceHandler) / sample.length;
  out = (out > 0 && Math.sqrt(out));
...
statistic.variance = varianceHandler;
